...
import { useGetAllProductsQuery, useGetProductBySlugQuery } from "../store/api/seller/productApi";
...
const ProductDetails = () => {
  ...
  try {
    const response = await fetch(`https://apiv2.shiprocket.in/v1/external/courier/serviceability/?pickup_postcode=110001&delivery_postcode=${pincode}&cod=1&weight=0.5`, {
      headers: {
    ...
    if (data.data?.available_courier_companies?.length > 0) {
      ...
      setDeliveryInfo({ message: "Delivery not available for this pincode " });
    }
  } catch (err) {
    console.error(err);
