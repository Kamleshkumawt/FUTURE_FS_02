...
const ShowAllDeliveredOrders = () => {
  ...
  return !isLoading && (
    <>
      <Title text1="Cancelled Orders" text2="List" />
        ...
        <table className="w-full border-collapse rounded-md overflow-hidden text-nowrap">
          ...
          <tbody className="text-sm font-light">
            {orders.map((item, index) => (
              ...
              >
                <td className="p-2 pl-10 flex gap-2 items-center ">{item.items.map((item) => <><img className="w-10 rounded" src={item.thumbnail} alt="" /></>)}</td>
