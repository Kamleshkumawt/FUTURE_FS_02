...
const ShowAllShippedOrders = () => {
   const [orders, setOrders] = useState([]);
    ...
    useEffect(() => {
      if(data) {
        ...
        const filteredOrders = data.orders.filter(order => order.status === 'Shipped');
  ...
  return !isLoading && (
    <>
      <Title text1="Shipped Orders" text2="List" />
          ...
          <tbody className="text-sm font-light">
            {orders.map((item, index) => (
              ...
              >
                ...
                <td className="p-2 ">{item.items.map((item) => item.title)}</td>
...
export default ShowAllShippedOrders